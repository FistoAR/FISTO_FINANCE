  ...
  try {
    ...
    if (result.success && result.reports.length > 0) {
      result.reports.forEach(report => {
        ...
        // ✅ Load saved projects as tags
        if (report.project_name) {
          const key = `${report.employee_id}-${dateStr}`;
          selectedProjects[key] = report.project_name.split(',').map(p => p.trim());
          
          const container = card.querySelector(`.project-tags-container[data-employee="${report.employee_id}"][data-date="${dateStr}"]`);
          const input = container ? container.querySelector('.project-tags-input') : null;
          
          if (container && input) {
            renderProjectTagsInside(key, container, input);
          }
        }
      });
...
function createReportCard(date, index) {
  ...
  card.innerHTML = `
    ...
    <div class="report-content">
      <div class="tasks-table-wrapper">
        <table class="tasks-table">
          ...
          <tbody>
            ${employees.length === 0 ? 
              '<tr><td colspan="7" style="text-align: center; padding: 2rem; color: #999;">No employees found</td></tr>' :
              employees.map(emp => `
                <tr>
                  ...
                  <td>
                     <div class="project-tags-input-wrapper">
    <div class="project-tags-container" data-employee="${emp.id}" data-date="${dateStr}">
      <input 
        type="text" 
    ...
    <div class="project-suggestions" style="display: none;"></div>
  </div>
                  </td>
                  <td>
                    <input 
...
document.addEventListener('click', function(e) {
  if (!e.target.closest('.project-tags-input-wrapper')) {
...
// Auto-resize input as tags are added
dailyReportsContainer.addEventListener('DOMNodeInserted', function(e) {
  if (e.target.classList && e.target.classList.contains('project-tag')) {
    const container = e.target.parentElement;
    const input = container.querySelector('.project-tags-input');
    if (input) {
...
dailyReportsContainer.addEventListener('click', function(e) {
  if (e.target.closest('.remove-project-tag')) {
    const btn = e.target.closest('.remove-project-tag');
    const key = btn.getAttribute('data-key');
    const projectName = btn.getAttribute('data-project');
    
    if (selectedProjects[key]) {
      selectedProjects[key] = selectedProjects[key].filter(p => p !== projectName);
      ...
      // If no more tags, reset input appearance
      if (selectedProjects[key].length === 0) {
...
dailyReportsContainer.addEventListener('keydown', function(e) {
  if (e.target.classList.contains('project-tags-input')) {
    const input = e.target;
    
    // On Enter or comma, close suggestions
    if (e.key === 'Enter' || e.key === ',') {
      e.preventDefault();
      const wrapper = input.closest('.project-tags-input-wrapper');
      const suggestionsDiv = wrapper.querySelector('.project-suggestions');
      
      // If suggestions visible, select first one
      const firstSuggestion = suggestionsDiv.querySelector('.project-suggestion-item');
    ...
    if (e.key === 'Backspace' && input.value === '') {
      ...
      if (selectedProjects[key] && selectedProjects[key].length > 0) {
        selectedProjects[key].pop();
        const container = input.closest('.project-tags-container');
        const lastTag = container.querySelector('.project-tag:last-of-type');
...
dailyReportsContainer.addEventListener('input', async function(e) {
  if (e.target.classList.contains('project-tags-input')) {
    const input = e.target;
    ...
    try {
      const response = await fetch(`${API_BASE}/search_projects.php?query=${encodeURIComponent(query)}`);
      const result = await response.json();
      
      if (result.success && result.projects.length > 0) {
        displayProjectSuggestions(result.projects, suggestionsDiv, input);
      } else {
        suggestionsDiv.style.display = 'none';
      }
    } catch (error) {
  ...
  // Keep existing percentage validation
...
function displayProjectSuggestions(projects, suggestionsDiv, input) {
  suggestionsDiv.innerHTML = projects.map(proj => `
    <div class="project-suggestion-item" data-project="${escapeHtml(proj.project_name)}">
  ...
  suggestionsDiv.querySelectorAll('.project-suggestion-item').forEach(item => {
    item.addEventListener('click', function() {
      const projectName = this.getAttribute('data-project');
      const employeeId = input.getAttribute('data-employee');
      const date = input.getAttribute('data-date');
      
      addSelectedProject(employeeId, date, projectName, input);
...
function addSelectedProject(employeeId, date, projectName, inputElement) {
  const key = `${employeeId}-${date}`;
  ...
  if (selectedProjects[key].includes(projectName)) {
    CommonModal.error('Project already added', 'Duplicate');
    return;
  }
  
  selectedProjects[key].push(projectName);
  
  // Update UI - render tags inside the container
  const container = inputElement.closest('.project-tags-container');
  renderProjectTagsInside(key, container, inputElement);
}



function renderProjectTagsInside(key, container, inputElement) {
  // Remove existing tags
  container.querySelectorAll('.project-tag').forEach(tag => tag.remove());
  
  if (!selectedProjects[key] || selectedProjects[key].length === 0) {
    return;
  }
  
  // Create tags before the input
  selectedProjects[key].forEach(project => {
    const tag = document.createElement('div');
    tag.className = 'project-tag';
    tag.innerHTML = `
      <span>${escapeHtml(project)}</span>
      <button type="button" class="remove-project-tag" data-key="${key}" data-project="${escapeHtml(project)}">
  ...
  if (e.target.classList.contains('save-btn')) {
    ...
    inputs.forEach(input => {
      ...
      if (value && type !== 'project') {
        hasData = true;
        employeeData[employeeId][type] = value;
      }
    });
    
    // Add selected projects to employee data
    Object.keys(employeeData).forEach(employeeId => {
      const key = `${employeeId}-${date}`;
      if (selectedProjects[key] && selectedProjects[key].length > 0) {
        employeeData[employeeId].project = selectedProjects[key].join(', ');
        hasData = true;
      }
    });
